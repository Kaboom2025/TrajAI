...
def sample_trajectory() -> Trajectory:
    steps = [
        TrajectoryStep(0, "tool_call", 100.0, tool_name="search", tool_args={"q": "test"}, tool_result="results"),
        TrajectoryStep(1, "llm_call", 101.0, model="gpt-4"),
        TrajectoryStep(2, "tool_call", 102.0, tool_name="calculator", tool_args={"expression": "1+1"}, tool_result="2"),
        TrajectoryStep(3, "tool_call", 103.0, tool_name="search", tool_args={"q": "more"}, tool_result="even more results"),
    ]
...
def test_tool_called_with(sample_trajectory: Trajectory) -> None:
    ...
    passed, msg = tool_called_with(sample_trajectory, "search", q="test", extra="none")
    assert passed is False
...
def test_tool_called_with_partial(sample_trajectory: Trajectory) -> None:
    ...
    steps = [
        TrajectoryStep(0, "tool_call", 100.0, tool_name="email", tool_args={"to": "a@b.com", "body": "hi"}),
    ]
    ...
    passed, msg = tool_called_with_partial(traj, "email", to="a@b.com", body="wrong")
    assert passed is False
...
def test_tool_called_immediately_before(sample_trajectory: Trajectory) -> None:
    ...
    passed, msg = tool_called_immediately_before(sample_trajectory, "search", "calculator")
    assert passed is True
    
    passed, msg = tool_called_immediately_before(sample_trajectory, "calculator", "search")
    assert passed is True # calculator(2) is immediately before search(3)
    
    passed, msg = tool_called_immediately_before(sample_trajectory, "search", "search")
...
def test_call_order_contains(sample_trajectory: Trajectory) -> None:
    passed, msg = call_order_contains(sample_trajectory, ["search", "calculator"])
    assert passed is True
    
    passed, msg = call_order_contains(sample_trajectory, ["search", "search"])
    assert passed is True
    
    passed, msg = call_order_contains(sample_trajectory, ["calculator", "missing"])
    assert passed is False
